...
class PgSqlKey {
  ...
  ///
  /// When `true`, generated `toPgSql` omits the PgSQL key only when the **Dart**
  /// field is `null`, but still writes `"key": null` when the field is
  /// non-null and serialization yields PgSQL `null`.
  ///
  /// Generated `fromPgSql` uses [Map.containsKey] to distinguish a missing key
  /// (typically decoded as Dart `null` on a nullable field) from an explicit
  /// PgSQL `null` (decoded via the field's `fromPgSql` path with a `null`
  /// argument). The field type must be nullable, and any custom [fromPgSql]
  /// function must accept a nullable PgSQL input for the explicit-`null` case.
  ///
  ...
  /// same field, an exception will be thrown during code generation.
  ///
  /// If [explicitPgSqlNullWhenNonNullField] is `true`, this value is ignored
  ...
  ///
  /// Cannot be combined with [explicitPgSqlNullWhenNonNullField].
  ...
  const PgSqlKey({
    ...
    this.explicitPgSqlNullWhenNonNullField,
    this.fromPgSql,
    @Deprecated(
      'Use `includeFromPgSql` and `includeToPgSql` with a value of `false` '
      'instead.',
