...
class _FieldSet implements Comparable<_FieldSet> {
  ...
  factory _FieldSet(FieldElement? classField, FieldElement? superField) {
    ...
    // Prefer the field that's annotated with `PgSqlKey`, if any.
    // If not, use the class field.
    final fieldHasPgSqlKey =
        fields.firstWhere(hasPgSqlKeyAnnotation, orElse: () => fields.first);

    return _FieldSet._(fieldHasPgSqlKey, sortField);
  }
