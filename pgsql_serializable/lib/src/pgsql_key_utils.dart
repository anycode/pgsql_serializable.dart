...
/// Will log "info" if [element] has an explicit value for [PgSqlKey.nullable]
/// telling the programmer that it will be ignored.
void logFieldWithConversionFunction(FieldElement element) {
  final pgSqlKey = _pgSqlKeyExpando[element];
  if (_explicitNullableExpando[pgSqlKey] ?? false) {
    log.info(
    ...
    _explicitNullableExpando[pgSqlKey] = null;
  }
}
...
PgSqlKey _populatePgSqlKey(
  ...
  _explicitNullableExpando[PgSqlKey] = nullable != null;
