...
class _ReadmeBuilder extends Builder {
  ...
  FutureOr<void> build(BuildStep buildStep) async {
    ...
    final pgsqlAnnotationVersion = await buildStep.pgsqlAnnotationVersion();
    final pgsqlAnnotationBaseUri =
        'https://pub.dev/documentation/pgsql_annotation/$pgsqlAnnotationVersion/pgsql_annotation';

    String pgsqlAnnotationUri(String className, [String? member]) =>
        member == null
        ? '$pgsqlAnnotationBaseUri/$className-class.html'
        : '$pgsqlAnnotationBaseUri/$className/$member.html';
    ...
    final theMap = <Pattern, String Function(Match)>{
      ...
      RegExp(r'`(\w+):(\w+)(\.\w+)?`'): (match) {
        ...
        linkValue = switch (context) {
          ...
          'ja' => pgsqlAnnotationUri(className, memberName?.substring(1)),
...
extension on BuildStep {
  ...
  Future<String> pgsqlAnnotationVersion() async {
    final pgsqlAnnotationPubspecAssetId = AssetId(
      'pgsql_annotation',
      'pubspec.yaml',
    );
    final pgsqlAnnotationPubspecContent = await readAsString(
      pgsqlAnnotationPubspecAssetId,
    );
    final pubspecYaml =
        loadYaml(
              pgsqlAnnotationPubspecContent,
              sourceUrl: pgsqlAnnotationPubspecAssetId.uri,
            )
            as YamlMap;
    final pgsqlAnnotationVersionString = pubspecYaml['version'] as String;

    final pgsqlAnnotationVersion = Version.parse(
      pgsqlAnnotationVersionString.trim(),
    );
